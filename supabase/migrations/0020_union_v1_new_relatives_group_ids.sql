-- ============================================================
-- Union v1 · Multi-group memberships for inline new relatives.
--
-- The p_new_partner and p_new_children jsonb payloads already
-- carried a single guest_group text (the primary text label). To
-- match the main add-guest form's GroupPicker — chip picker with
-- multi-select — we widen the payload with an optional
-- guest_group_ids array of uuids. Each id is validated against
-- the wedding and becomes a guest_group_members row.
--
-- guest_group (text) still sets the primary text label on the
-- guests row; guest_group_ids drives the multi-membership. The
-- client always sends the first-selected group's name as the text.
-- ============================================================
